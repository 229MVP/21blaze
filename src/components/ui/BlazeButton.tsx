...
import { colors, radii, spacing, typography } from '../../theme/uiKit';
...
export function BlazeButton({
  ...
  const gradient =
    variant === 'danger' ? DANGER_GRADIENT : PRIMARY_GRADIENT;
  ...
  const content = (
    <View style={styles.content}>
      ...
      <Text
        style={[
          styles.label,
          variant === 'primary' ? styles.dark : styles.light,
        ]}
      >
        {label}
      </Text>
  ...
  return (
    ...
    >
      {variant === 'primary' || variant === 'danger' ? (
        <LinearGradient colors={gradient} style={[styles.base, styles[size]]}>
          {content}
        </LinearGradient>
      ) : (
        <View
...
const styles = StyleSheet.create({
  ...
  ghost: { backgroundColor: 'transparent', borderColor: colors.border.default },
  content: {
    flexDirection: 'row',
    gap: spacing.sm,
    alignItems: 'center',
